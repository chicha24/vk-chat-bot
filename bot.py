...
def appendUserJSON(newData, chatID):
    with open('chatLists.json', 'r+') as cl:
        dataLoaded = json.load(cl)
                ...
                for u in users['items']:
                    if u['member_id'] == event.message.from_id:
                        ...
                        if admin:
                            ...
                            try:
                                vk.messages.removeChatUser(chat_id = event.chat_id, user_id = cleanUserId)
                            except ApiError:
                ...
                for u in users['items']:
                    if u['member_id'] == event.message.from_id:
                        ...
                        if admin:
                            ...
                            appendUserJSON(data, event.chat_id)
                            sendMessage('{} причислен к списку пидорасов'.format(getUserName(repliedMessage['from_id'], vk)), event.chat_id)
                        else:
                            sendMessage('Недостаточно прав', event.chat_id)
            if "чича покажи пидорасов" in event.message.text.lower():
                faglist = []
                with open('chatLists.json', 'r') as cl:
                    data = json.load(cl)
